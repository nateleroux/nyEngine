...
#define EXCEPTION( msg, funcName ) dbgError("%s | %s", funcName, msg)
...
void ConfigScriptLoader::parseScript(DataStreamPtr &stream, map_t *parentMap)
{
...
void ConfigScriptLoader::purgeMap(map_t *map)
{
	stdext::hash_map<String, ConfigNode*>::iterator i;
    for (i = scriptList.begin(); i != scriptList.end();)
	{
		if(i->second->parentMap == map)
		{
			delete i->second;
			i = scriptList.erase(i);
		}
		else
			i++;
    }
        ...
        switch (tok){
            ...
            case TOKEN_Text:
                ...
                if (parent)
                    ...
                    newNode = new ConfigNode(0, pMap, tokVal);
...
ConfigNode::ConfigNode(ConfigNode *parent, map_t *map, const String &name)
{
...
ConfigNode *ConfigNode::addChild(const String &name, bool replaceExisting)
{
    ...
    return new ConfigNode(this, parentMap, name);
}
